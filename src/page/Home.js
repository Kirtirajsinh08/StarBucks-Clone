...
import { HandcraftedCurations, BaristaRecommends, cartdata } from '../database'
...
const Home = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/introdution')
      .then(function (response) {
    ...
    <>
          ...
          <div className='flex justify-between items-center text-center flex-wrap'>
            {
              HandcraftedCurations.map((el, index) => {
                return (
                  <Link to={"bestseller"} className="flex justify-center items-center flex-col" key={el.title + index}>
      ...
      <div className='bg-white py-14'>
        ...
        <div className='px-4 sm:px-0 md:w-4/5 m-auto relative'>
          <h1 className='text-[#1e3932] text-2xl font-bold mb-4'>Daily Offer</h1>
          {
            data.map((item) => {
              return (
                <>
                  {/* <h1 className='text-[#1e3932] text-2xl font-bold'>Daily offers</h1> */}
                  <div className='p-5 bg-[#0e382c] min-w-full rounded flex sm:gap-7 relative overflow-hidden'>
                    ...
                    <div className='text-white max-w-2xl'>
                      ...
                      <h2 className='font-bold text-base sm:text-xl my-2'>{item.name}</h2>
                      <p className='font-medium text-xs sm:text-base w-full'>{item.email}</p>
                      <p className='md:mt-5 text-xs sm:text-base'>{item.city}</p>
                      <p className='font-bold text-lg'>{item.state}</p>
                    </div>
                    <p className='text-white text-xs font-bold ml-auto absolute right-5'>{item.zip}</p>
                ...
                return (
                  <div key={el.id} className="overflow-hidden rounded-xl border shadow-md min-w-[280px]">
                    ...
                    <div className='p-5'>
                      ...
                      <h2 className='font-semibold text-[#1e3932]'>{el.title}</h2>
